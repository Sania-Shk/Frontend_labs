...
let displayCard = document.querySelector(".card-wrapper");
...
function data_Storage() {
  let userDetail = JSON.parse(localStorage.getItem("Data")) ?? [];
  userDetail.push({
    id: Date.now(),
    detailName: user_name.value,
    detailNumber: user_number.value,
    detail_UserExpiry: user_expiry.value,
    detail_UserCVC: user_cvc.value,
  });
  ...
  userDetail.forEach((element) => {
    miniCard += `
      ...
      <div class="card-art-surface">
       <button id="cancelBtn" onclick="cancelCard()">x</button>
        <div class="surface-branding">
          <img src="asserts/icon.png" class="issuer-logo" />
          <h2 class="network_type">VISA</h2>
        </div>

        <div class="card-data-primary">
          <h2 id="card-number-mask" class="number-display">
            ${element.detailNumber}
          </h2>

          <div class="card-data-secondary">
            <div class="field-group">
              <h4 class="field-label">CARDHOLDER</h4>
              <p id="card-name-output" class="field-value">${element.detailName}</p>
            </div>

            <div class="meta-flex-container">
              <div class="meta-field">
                <h4 class="field-label">EXPIRY</h4>
                <p id="expiry-output" class="field-value">${element.detail_UserExpiry}</p>
              </div>

              <div class="meta-field">
                <h4 class="field-label">CVC</h4>
                <p id="cvc-output" class="field-value">${element.detail_UserCVC}</p>
              </div>

              <img src="asserts/chip.jpeg" class="emv-chip-icon" />
            </div>
          </div>
        </div>
      </div>
  ...
  `;
    ...
    displayCard.innerHTML = miniCard;
  });
